...
def get_cis_dist(read1, read2):
    ...
    if not r1.is_unmapped and not r2.is_unmapped:         
        ...
        if isIntraChrom(read1, read2):
            r1pos = int(get_read_pos(read1))
            r2pos = int(get_read_pos(read2))
...
def get_valid_pair(r1, r2, r1_chrom, r2_chrom, r1_frag, r2_frag, r1_bin, r2_bin, cisDist):
    ...
    validInfo = "\t".join([or1.qname, or1_chrom, str(int(get_read_pos(or1))), get_read_strand(or1), or1_fragname, str(or1_bin),\
                           or2_chrom, str(int(get_read_pos(or2))), get_read_strand(or2), or2_fragname, str(or2_bin),\
...
if __name__ == "__main__":
        ...
        if read.is_read1:
            ...
            if r1 is not None and r2 is not None:
                ...
                if len(validInfoList) == 1:
                    outValid.write(validInfoList[0] + "\n")
                elif len(validInfoList) > 1:
                    print("Warning! # of valid Read-pairs is more than 1!")
    ...
    if args.summary:
        ...
        outSummary.write("Total number of read pairs:\t" + str(uni_count) + "\n")
